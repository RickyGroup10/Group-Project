...
To help us answer our question, we’ve used cluster analysis to classify groups of observations that get lots of likes and those that don’t using k-means and to see which numerical variables get the best results.

For this we used the normalised and outlier removed dataset.

From the skree plot, the optimal number of clusters is 2.
...
```{r CATable, results = 'asis', echo=FALSE}
print(xtable(aggregate(no_outliers[1:4], by=list(cluster=result$cluster), mean)), 
      caption='Cluster Analysis Output', label='CA', caption.placement="top", type="latex", comment=FALSE)
...
As we can see in table \@ref(pcasum), two principal components only explain 56% of the variance and three explains 78%. This is visualised in figure \@ref(pcvarplot). We will need to use three principal components for predictions as this explains the majority. 
...
# NEED TO SORT THIS** I think we need each section's individual conclusions to write this?

If we had more and/or different measures available to us, we may be able to deduce what features attract profile visits and, therefore, more kisses. With the data we have access to as of now, we can only conclude that love is unpredictable
