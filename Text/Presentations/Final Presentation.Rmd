...
header-includes:
  - \setbeamertemplate{footline}{\includegraphics[height=15px]{"../../Images/PNG/Hearts_final.png"}}
...
## Profile Visits and Profile Likes
...
lm1 <- lm(counts_kisses ~ age, data = lovoo)
lm2 <- lm(counts_kisses ~ counts_pictures, data = lovoo)
lm3 <- lm(counts_kisses ~ counts_profileVisits, data = lovoo)
lm4 <- lm(counts_kisses ~ distance, data = lovoo)
par(mfrow=c(2,2))
plot(lm1, 1)
plot(lm2, 1)
plot(lm3, 1)
...
"What combination of features lead to more likes and a better dating app experience?"

## Introduction to LDA

-	Why and how we are using LDA

![]("../../Images/PNG/lda_pairs.png")

## Conducting LDA

![]("../../Images/PNG/lda_mean.png")

## LDA Results

![]("../../Images/PNG/lda_hist.png")

## LDA Results continued

![]("../../Images/PNG/lda_biplot.png")

## Using Reduced Dimension for Classification

-	Accuracy: 0.53

![]("../../Images/PNG/lda_predact.png")

## Problems we encounted

- Our data was not normal
- A large number of outliers
- No linear separation between groups
- Failed most of our tests
